...
public class ServerHandler implements Runnable {
	...
	public final String game_timeout = "Vous avez été trop lent... Vous avez donc perdu.";
	...
	// Changer le messageCreateListener de l'API vers le serveur.
	// Pour cela, stocker le serveur, et non plus seulement son Id.
    ...
    private boolean verifyStartGameMessage(List<User> listUser, User author, TextChannel channel) {
    	if(grid != null) {
    		channel.sendMessage(error_opengame);
    		return false;
    	}
    	if(listUser.isEmpty()) {
			channel.sendMessage("Il n'est pas encore possible de jouer face à moi... Merci de choisir un adversaire.");
			return false;
		}
    	if (author.isBot() || listUser.get(0).isBot()){
			channel.sendMessage(error_bot);
			return false;
		}
    	if (listUser.size() > 1){
			channel.sendMessage(error_toomanyplayers);
			return false;
		}
    	return true;
    }
    	...
    	if(grid == null) {
    		...
    		try {
	    		if(event.getMessageAuthor().getId() == nextPlayer.getId()) {
	    			timertask.cancel();
	    			int result = grid.play(nextSymbol, Integer.parseInt(data[1]), Integer.parseInt(data[2]));
	    			...
	    			if(event.getMessageAuthor().getId() != nextPlayer.getId()) {
	    				event.getChannel().sendMessage(error_notyourturn);
	    			}
	    		}
